...
tools/              # 산출물 생성 스크립트 (Word, HTML, Excel 등)
outputs/            # 완성 산출물 (클라이언트 전달 파일)
assets/             # 디자인 리소스
  └── figma/        # Figma export 이미지, 아이콘 등
.tmp/               # 작업 중 임시 파일 (언제든 삭제 가능)
.env                # API 키, Figma 토큰 등 민감 데이터
```

**클라이언트에게 전달하는 것**: `outputs/` 폴더 안의 최종 파일만.
**로컬에만 남는 것**: `.tmp/`는 일회용이야. 삭제해도 중요한 건 없어.
