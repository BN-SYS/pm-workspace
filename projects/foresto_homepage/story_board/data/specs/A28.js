...
window.ANNOTATIONS['A28'] = [
  { n: 1, x: 90, y:  7 },   // 상단 버튼 ([+ 항목 추가])
  { n: 2, x: 13, y: 11 },   // 목록 테이블
  { n: 3, x: 88, y: 22 },   // 행 내 관리 버튼 (수정·삭제)
];
...
<p>
  ...
  - [+ 연도 추가]: 연혁 등록/수정(A29) 화면으로 이동 (등록 모드)
</p>
...
<p>
  ...
  - 행 클릭: 해당 연도 수정 (연혁 등록/수정, A29, ?year= 전달)<br>
  - 내용 컬럼: 항목별 줄바꿈 표시 (text 말줄임 없이 전체 표시)
</p>

<h4>③ 행 내 관리 버튼</h4>
<p>
  - [수정]: A29로 이동 (?year= 전달)<br>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>3</td><td>연도</td><td>number</td><td>C</td><td>굵게 표시. 클릭 시 A29(수정) 이동</td></tr>
...
<table>
  ...
  <tbody>
    <tr><td>A29</td><td>연혁 등록/수정</td><td>[+ 연도 추가] 클릭 (등록)/행 클릭 또는 [수정] 클릭 (수정, ?year= 전달)</td></tr>
