...
window.ANNOTATIONS['A08'] = [
  ...
  { n: 2, x: 13, y: 35 },   // 목록 테이블
  { n: 3, x: 13, y: 56 },   // 하단 버튼 ([선택 삭제])
];
...
<!-- ── 목적 ──────────────────────────────────────── -->
<h3>목적</h3>
<p>관리자가 전체 강좌의 신청자를 통합 조회하고, 과정유형·신청상태·수료상태 등 조건으로 검색·필터링하여 신청 현황을 관리하는 화면.</p>
...
<p>
  ...
  - 과정유형 필터: 버튼 그룹 토글(전체/기초과정/자격취득과정/역량강화/회원아카데미). 기본값 "전체". ⚠️ 과정유형 목록 확정 필요<br>
  - 신청상태 필터: 버튼 그룹 토글(전체/신청완료/취소). 기본값 "전체". ⚠️ 신청상태 값 확정 필요<br>
  - 수료상태 필터: 버튼 그룹 토글(전체/수료/미수료). 기본값 "전체". ⚠️ 수료상태 값 확정 필요<br>
  - 검색어 조건: 셀렉트박스(과정명+신청자/과정명/신청자 등). ⚠️ 검색 조건 항목 확정 필요<br>
...
<p>
  ...
  - 행 클릭: 신청 상세(A09)로 이동. ⚠️ 상세 없이 목록에서만 관리하는 경우 행 클릭 동작 없음 — 협의 필요<br>
...
<p>
  ...
  - [신청 상태변경]: 우측 배치. 선택된 신청자 상태 일괄 변경. ⚠️ 상태 변경 가능 값 및 프로세스 협의 필요
</p>
<p class="spec-note">⚠️ 삭제 방식 협의 필요 — Hard Delete vs. Soft Delete. 수료 완료 신청 건 삭제 차단 여부 협의 필요.</p>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>3</td><td>과정유형</td><td>badge</td><td>C</td><td>기초과정/자격취득과정/역량강화/회원아카데미. ⚠️ 유형별 색상 확정 필요</td></tr>
    ...
    <tr><td>8</td><td>핸드폰</td><td>text</td><td>C</td><td>010-XXXX-XXXX. ⚠️ 마스킹 적용 여부 협의 필요</td></tr>
    ...
    <tr><td>11</td><td>신청상태</td><td>badge</td><td>C</td><td>신청완료/취소 등. ⚠️ 상태값·색상 확정 필요</td></tr>
    <tr><td>12</td><td>수료상태</td><td>badge/text</td><td>C</td><td>수료/미수료/"—"(해당 없음). ⚠️ 수료상태 값 확정 필요</td></tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>상태변경 완료</td><td>Toast</td><td>상태가 변경되었습니다.</td></tr>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>A06</td><td>강좌 상세</td><td>과정명 클릭 시 이동 가능 ⚠️ 연결 여부 협의 필요</td></tr>
